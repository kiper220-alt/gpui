...
namespace
{
const QString kSystemDrive = QStringLiteral("%SystemDrive%");
...
{
    selectOrType(ui->periodicComboBox,
                 extras.value(QStringLiteral("freeSpace"), QStringLiteral("80")));
    const QString drive = extras.value(QStringLiteral("drive"), kSystemDrive);
    if (drive == kSystemDrive)
    {
        ui->comboBox->setCurrentIndex(0);
        return;
    }
    const int idx = ui->comboBox->findText(drive);
    if (idx >= 0)
    {
...
{
    const QString drive = ui->comboBox->currentIndex() == 0
                            ? kSystemDrive
