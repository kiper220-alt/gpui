...
QString formatProcessingMode(const QMap<QString, QString> &e)
{
    const QList<QPair<QString, const char *>> flags = {
        {QStringLiteral("synchFore"),  "synchronous foreground"},
        {QStringLiteral("asynchFore"), "asynchronous foreground"},
    ...
    static const QHash<QString, Formatter> t = {
        ...
        {QStringLiteral("FilterRegistry"),
            [](const QMap<QString, QString> &e) {
                ...
                if (!val.isEmpty())
                {
                    return TR("registry value %1 in %2\\%3 matches").arg(val, hive, key);
                }
                return TR("registry key %1\\%2 exists").arg(hive, key);
            }},
