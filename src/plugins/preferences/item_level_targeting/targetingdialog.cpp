...
public:
    ...
    {
        return {QString::fromLatin1(kClipboardMimeType)};
    }
    ...
    QMimeData *mimeData(const QModelIndexList &indexes) const override
    {
        ...
        for (auto *item : items)
        {
            out << static_cast<qulonglong>(reinterpret_cast<quintptr>(item));
        }
        mime->setData(QString::fromLatin1(kClipboardMimeType), data);
        return mime;
    }
...
private:
                     ...
                     int &insertAt) const
    {
        ...
        for (quint32 i = 0; i < count; ++i)
        {
            qulonglong raw = 0;
            in >> raw;
            auto *item = reinterpret_cast<TargetingFilterItem *>(static_cast<quintptr>(raw));
        ...
        for (auto *source : sources)
        {
            if (isAncestorOf(source, targetParent))
            {
                return false;
            }
            for (auto *other : sources)
            {
                if (source != other && isAncestorOf(source, other))
                {
                    return false;
                }
            }
    ...
    static bool isAncestorOf(const ModelView::SessionItem *ancestor,
    ...
    static int depth(const ModelView::SessionItem *item)
    {
        int d = 0;
        for (auto *p = item; p; p = p->parent())
        {
